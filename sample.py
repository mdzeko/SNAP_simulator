...
def main():
    ...
    with open(inputZavisnosti, "r") as csvZavisnosti:
        zavisnostiReader = csv.reader(csvZavisnosti, delimiter=';')
        listaZavisnosti = list(zavisnostiReader)

    n = int((brojKriterija - 1) * brojKriterija / 2)
    usporedba = list(np.ones(n))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "-help", "--help"):
        print("main.py <broj_kriterija> <datotekaZavisnost> <brojDretvi> <direktorijIzlaza>")
        print("PRIMJER: python main.py ~/usporedbe.csv ~/zavisnosti.csv 4 /shared/mdzeko")
        sys.exit(2)
    brojKriterija, inputZavisnosti, brojDretvi, outDir = tuple(sys.argv[1:])
